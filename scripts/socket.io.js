...
debug.enable = function(name) {
  try {
    chrome.storage.local.debug = name;
  } catch(e){}
...
try {
  if (chrome.storage.local) debug.enable(chrome.storage.local.debug);
} catch(e){}
...
function save(namespaces) {
  try {
    if (null == namespaces) {
      chrome.storage.local.removeItem('debug');
    } else {
      chrome.storage.local.debug = namespaces;
    }
  } catch(e) {}
}
